...
func main() {
	slog.Info("starting Instagram data fetch batch")
	...
	instagramConnRepo := repository.NewInstagramConnectionRepository(dbDriver)
	instagramDailyReportRepo := repository.NewInstagramDailyReportRepository(dbDriver)
	instagramDataAdapter := adapter.NewInstagramDataAdapter()
	instagramFetchUseCase := usecase.NewInstagramFetchUseCase(instagramConnRepo, instagramDailyReportRepo, instagramDataAdapter)
	...
	if err := instagramFetchUseCase.Execute(ctx, targetDate); err != nil {
		slog.Error("batch execution failed", "error", err)
		os.Exit(1)
	}

	slog.Info("Instagram data fetch batch completed")
}
