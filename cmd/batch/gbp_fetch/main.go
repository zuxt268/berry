...
func main() {
	slog.Info("starting GBP data fetch batch")
	...
	// 依存組み立て
	gbpConnRepo := repository.NewGBPConnectionRepository(dbDriver)
	gbpDailyReportRepo := repository.NewGBPDailyReportRepository(dbDriver)
	gbpDataAdapter := adapter.NewGBPDataAdapter()
	gbpFetchUseCase := usecase.NewGBPFetchUseCase(gbpConnRepo, gbpDailyReportRepo, gbpDataAdapter)
	...
	// 実行
	if err := gbpFetchUseCase.Execute(ctx, targetDate); err != nil {
		slog.Error("batch execution failed", "error", err)
		os.Exit(1)
	}

	slog.Info("GBP data fetch batch completed")
}
